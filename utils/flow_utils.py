...
def load_flow(path):
    if path.endswith('.png'):
        ...
        # judge whether the flow is generated by self-rendering
        path_range = path.replace(path.name, 'data_ranges.json')
        if exists(path_range):
...
def plt_show_img_flow(img_list=[], flow_list=[], batch_index=0):
    ...
    for i in range(len(flow_list)):
        ...
        plot_quiver(ax1, flow=flow_vec[i], mask=flow_list[i], spacing=(40 * flow_list[i].shape[0]) // 512)
