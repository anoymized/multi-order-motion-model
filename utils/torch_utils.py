...
def bias_parameters(module):
    ...
    # add self-defined parm


def other_paramters(module):
...
def save_checkpoint(save_path, states, file_prefixes, is_best, error, filename='ckpt.pth.tar'):
    def run_one_sample(save_path, state, prefix, is_best, error, filename):
        torch.save(state, save_path / '{}_{}_{:.2f}_{}'.format(prefix, str(state['epoch']), error, filename))
        if is_best:
            shutil.copyfile(save_path / '{}_{}_{:.2f}_{}'.format(prefix, str(state['epoch']), error, filename),
                            save_path / '{}_model_best.pth.tar'.format(prefix))

    if not isinstance(file_prefixes, str):
        for (prefix, state) in zip(file_prefixes, states):
            run_one_sample(save_path, state, prefix, is_best, error, filename)

    else:
        run_one_sample(save_path, states, file_prefixes, is_best, error, filename)
