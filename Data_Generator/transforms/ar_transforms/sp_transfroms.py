...
class RandomAffineFlow(nn.Module):
    ...
    def apply_random_transforms_to_params(self,
        ...
        while invalid.sum() > 0:
            ...
            count += 1
            if count >= 5:
                zoom = torch.ones_like(zoom)
                squeeze = torch.ones_like(squeeze)
                tx = torch.zeros_like(tx)
                ty = torch.zeros_like(ty)
    ...
    def apply_random_transforms_to_params_sm(self,
                                             theta0,
                                             max_translate,
                                             min_zoom, max_zoom,
                                             min_squeeze, max_squeeze,
                                             min_rotate, max_rotate,
                                             validate_size=None, last_state=None):
        max_translate *= 0.5
        batch_size = theta0.size(0)
        height, width = validate_size
        ...
        while invalid.sum() > 0:
            ...
            if count >= 5:
                ...
                factor = 2.0
                zoom = torch.randn_like(zoom) * ((max_zoom - min_zoom) / 2) / factor + last_state[0]
                squeeze = torch.randn_like(squeeze) * ((max_squeeze - min_squeeze) / 2) / factor + last_state[1]
                tx = torch.randn_like(tx) * max_translate / factor + last_state[2]
                ty = torch.randn_like(ty) * max_translate / factor + last_state[3]
                phi = torch.randn_like(phi) * ((max_rotate - min_rotate) / 2) / factor + last_state[4]
            ...
            # compute new invalid ones
            invalid = self.find_invalid(width=width, height=height, thetas=thetas).float()
        last_state = [zoom, squeeze, tx, ty, phi]
        # here we should have good thetas within borders
        return thetas, last_state
    ...
    def forward(self, data):
        ...
        # relative transform
        last_state = None
        if not self.cfg.st_sm:
            for i in range(len(imgs) - 1):
                theta = self.apply_random_transforms_to_params(
                    theta_list[-1],
            ...
            for i in range(len(imgs) - 1):
                theta, last_state = self.apply_random_transforms_to_params_sm(
                    theta_list[-1],
                    max_translate=self.cfg.trans[1],
                    min_zoom=self.cfg.zoom[2], max_zoom=self.cfg.zoom[3],
                    min_squeeze=self.cfg.squeeze[2], max_squeeze=self.cfg.squeeze[3],
                    min_rotate=self.cfg.rotate[2], max_rotate=self.cfg.rotate[3],
                    validate_size=[height, width], last_state=last_state)
                theta_list.append(theta)
        ...
        flows_f = [self.transform_flow(flo, theta1, theta2) for flo, theta1, theta2 in
                   zip(flows_f, theta_list[0:-1], theta_list[1:])]

        if "mask" in data:
            mask = [self.transform_image(mask, theta) for mask, theta in
                    zip(data["mask"], theta_list)]
            data["mask"] = mask
