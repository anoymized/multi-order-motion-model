...
def mod_skew(im, sk):
    ...
    if _sd > 0:
        ...
        _A = mu[3] - 3. * _sd * _sk * mu[2] + 3. * _sd ** 2. * (_sk ** 2. - 1.) * mu[1] + _sd ** 6 * (
                    2 + 3 * _sk ** 2 - _sk ** 4)
...
def mod_kurt(im, kt):
    ...
    if _sd > 0:
        ...
        _A = mu[9] - 4. * _a * mu[7] - 4 * mu[0] * mu[6] + 6. * _a ** 2 * mu[5] + 12 * _a * mu[0] * mu[4] + 6 * mu[
            0] ** 2 * mu[3] \
             - 4 * _a ** 3 * mu[3] - 12 * _a ** 2 * mu[0] * mu[2] + _a ** 4 * mu[1] - 12 * _a * mu[0] ** 2 * mu[1] \
             + 4 * _a ** 3 * mu[0] ** 2 + 6 * _a ** 2 * mu[0] ** 2 * _sd ** 2 - 3. * mu[0] ** 4
        _B = 4. * (mu[7] - 3 * _a * mu[5] - 3 * mu[0] * mu[4] + 3. * _a ** 2 * mu[3] + 6. * _a * mu[0] * mu[2] + 3. *
                   mu[0] ** 2 * mu[1] \
                   - _a ** 3 * mu[1] - 3. * _a ** 2 * mu[0] ** 2 - 3 * mu[1] * mu[0] ** 2)
        _C = 6. * (mu[5] - 2. * _a * mu[3] - 2. * mu[0] * mu[2] + _a ** 2 * mu[1] + 2. * _a * mu[0] ** 2 + mu[
            0] ** 2 * _sd ** 2)
        ...
        mMnewKt = np.polyval(np.array([_A, _B, _C, _D, _E], dtype='double'), lam) / np.polyval(
            np.array([_F, 0, _G], dtype='double'), lam) ** 2
...
def pad_reflect(image):
    image1 = np.pad(image, [[int(image.shape[0] / 2), int(image.shape[0] / 2)],
                            [int(image.shape[1] / 2), int(image.shape[1] / 2)]], 'reflect')
    return image1
