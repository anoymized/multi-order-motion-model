...
class SteerablePyramid():
    def __init__(self, image, xres, yres, n, k, image_name, out_path, verbose):
        ...
        self.ALPHAK = 2. ** (self.K - 1) * math.factorial(self.K - 1) / np.sqrt(
            self.K * float(math.factorial(2. * (self.K - 1))))
        self.RES = []
        ...
        for i in range(self.N):
            ...
            for j in range(len(self.B_FILT[i])):
                ...
                if self.verbose == 1:
                    ...
                    Image.fromarray(np.uint8(_tmp), mode='L').save(
                        self.OUT_PATH.format('{}-layer{}-lb{}.png'.format(self.IMAGE_NAME, str(i), str(j))))
            ...
            if self.verbose == 1:
                _tmp = np.absolute(img_back)
                Image.fromarray(np.uint8(_tmp), mode='L').save(
                    self.OUT_PATH.format('{}-residual-layer{}.png'.format(self.IMAGE_NAME, str(i))))
