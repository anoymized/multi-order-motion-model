...
parser.set_defaults(save_state=False, frame_end=20, frame_rate=30,
                    resolution=768)
...
for scene_id in range(start_scenes, start_scenes + num_scenes):
    ...
    output_dir_s = 'specular' / output_dir
    output_dir_d = 'diffuse' / output_dir
    ...
    for i in range(num_static_objects):
        ...
        # obj_list_d.append(copy.deepcopy(obj))
        # obj = random_assign_material(obj)
        # obj_list_s.append(copy.deepcopy(obj))
    ...
    # first render the diffuse scene
    logging.info("Rendering the diffuse scene ...")
    ...
    logging.info("Rendering the specular scene ...")
    output_dir = output_dir_s
    scene._assets = list(scene.assets)
    obj_list = scene._assets[2:]
    for i in range(len(obj_list)):
        obj_list[i] = random_assign_material(obj_list[i])
